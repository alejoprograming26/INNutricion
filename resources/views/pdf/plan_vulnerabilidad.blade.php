    ...
    @if(count($datosAgrupados) > 0)
        <table class="report-table">
            <thead><tr>
                ...
                <th width="25%" class="text-center">Entregas Realizadas</th>
                <th width="25%" class="text-right">Total Insumos Entregados</th>
            </tr></thead>
            <tbody>
                @foreach($datosAgrupados as $mun => $dataMun)
                    ...
                    <tr class="level-municipio">
                        ...
                        <td class="text-right">{{ number_format($dataMun['totales']['total_entregas'],0,',','.') }}</td>
                    </tr>
                    @foreach($dataMun['parroquias'] as $par => $dataPar)
                        <tr class="level-parroquia">
                            ...
                            <td class="text-right">{{ number_format($dataPar['totales']['total_entregas'],0,',','.') }}</td>
                        </tr>
                        @foreach($dataPar['comunas'] as $com => $dataCom)
                            <tr class="level-comuna">
                                ...
                                <td class="text-right">{{ number_format($dataCom['totales']['total_entregas'],0,',','.') }}</td>
                            </tr>
                            @foreach($dataCom['sectores'] as $sec => $totales)
                                <tr class="level-sector">
                                    ...
                                    <td class="text-right">{{ number_format($totales['total_entregas'],0,',','.') }}</td>
                                </tr>
                            @endforeach
                        @endforeach
                    @endforeach
        ...
        <div class="grand-total clearfix"><table>
            ...
            <tr><th>Total Entregas Realizadas</th><td>{{ number_format($totalesGenerales['registros'],0,',','.') }}</td></tr>
            <tr><th>Total Insumos Entregados</th><td>{{ number_format($totalesGenerales['total_entregas'],0,',','.') }}</td></tr>
        </table></div>
    @else
