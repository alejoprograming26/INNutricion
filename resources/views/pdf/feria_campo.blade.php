...
<head>
    ...
    <style>
        ...
        .level-municipio td { background-color: {{ $colorThema }}; color: #ffffff; font-weight: bold; font-size: 10.5px; letter-spacing: 0.3px; padding-top: 7px; padding-bottom: 7px; border-top: 2px solid rgba(0,0,0,0.1); border-bottom: 1px solid rgba(0,0,0,0.2); }
        ...
        .grand-total table { width: 60%; float: right; border-collapse: collapse; }
...
<body>
    ...
    @if(count($datosAgrupados) > 0)
        <table class="report-table">
            <thead>
                <tr>
                    <th width="40%">Ubicación (Mun / Parr / Com / Sec)</th>
                    <th width="15%" class="text-center">Jornadas</th>
                    <th width="15%" class="text-right">Total A</th>
                    <th width="15%" class="text-right">Total B</th>
                    <th width="15%" class="text-right">Total A+</th>
                </tr>
            </thead>
            <tbody>
                @foreach($datosAgrupados as $mun => $dataMun)
                    @if(!$loop->first) <tr class="spacer-row"><td colspan="5"></td></tr> @endif
                    <tr class="level-municipio">
                        <td>&bull; MUNICIPIO: {{ $mun }}</td>
                        <td class="text-center">{{ number_format($dataMun['totales']['registros'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ number_format($dataMun['totales']['tipo_a'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ number_format($dataMun['totales']['tipo_b'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ number_format($dataMun['totales']['tipo_a_plus'], 0, ',', '.') }}</td>
                    </tr>
                    @foreach($dataMun['parroquias'] as $par => $dataPar)
                        <tr class="level-parroquia">
                            ...
                            <td class="text-right">{{ number_format($dataPar['totales']['tipo_a'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ number_format($dataPar['totales']['tipo_b'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ number_format($dataPar['totales']['tipo_a_plus'], 0, ',', '.') }}</td>
                        </tr>
                        @foreach($dataPar['comunas'] as $com => $dataCom)
                            <tr class="level-comuna">
                                ...
                                <td class="text-right">{{ number_format($dataCom['totales']['tipo_a'], 0, ',', '.') }}</td>
                                <td class="text-right">{{ number_format($dataCom['totales']['tipo_b'], 0, ',', '.') }}</td>
                                <td class="text-right">{{ number_format($dataCom['totales']['tipo_a_plus'], 0, ',', '.') }}</td>
                            </tr>
                            @foreach($dataCom['sectores'] as $sec => $totales)
                                <tr class="level-sector">
                                    ...
                                    <td class="text-right">{{ number_format($totales['tipo_a'], 0, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($totales['tipo_b'], 0, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($totales['tipo_a_plus'], 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        @endforeach
                    @endforeach
        ...
        <div class="grand-total clearfix">
            <table>
                ...
                <tr> <th>Total Jornadas Realizadas</th> <td>{{ number_format($totalesGenerales['registros'], 0, ',', '.') }}</td> </tr>
                <tr> <th>Suma Total A (Antropometría)</th> <td>{{ number_format($totalesGenerales['tipo_a'], 0, ',', '.') }}</td> </tr>
                <tr> <th>Suma Total B (Antropometría)</th> <td>{{ number_format($totalesGenerales['tipo_b'], 0, ',', '.') }}</td> </tr>
                <tr> <th>Suma Total A+ (Antropometría)</th> <td>{{ number_format($totalesGenerales['tipo_a_plus'], 0, ',', '.') }}</td> </tr>
            </table>
        </div>
    @else
