<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden">
        ...
        <div class="flex items-center gap-4 relative z-10">
            ...
            <div>
                <h1 class="text-3xl font-black text-zinc-800 dark:text-zinc-100 tracking-tight">Escuelas 4S</h1>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1 font-medium">
                    Certificación y seguimiento de planteles educativos.
                </p>
            </div>
        </div>
    ...
    <flux:card class="shadow-sm border-zinc-200/60 dark:border-zinc-800/60 !p-0 overflow-hidden">
        ...
        <div class="bg-zinc-50/80 dark:bg-zinc-800/30 p-4 border-b border-zinc-200 dark:border-zinc-800 flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div class="w-full lg:w-1/3">
                <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Buscar escuela, municipio, sector..." class="bg-white dark:bg-zinc-900 shadow-sm" />
            </div>
            ...
            <table class="w-full text-sm text-left text-zinc-600 dark:text-zinc-400">
                <thead class="bg-white dark:bg-zinc-900 text-xs uppercase font-semibold text-zinc-500 border-b border-zinc-200 dark:border-zinc-800">
                    <tr>
                        ...
                        <th class="px-6 py-4">Escuela / Responsable</th>
                        ...
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors group">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium mt-0.5">
                                            {{ $e->responsable }} &bull; {{ \Carbon\Carbon::parse($e->fecha)->format('d/m/Y') }}
                                        </p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1">
                                    <span class="font-medium text-zinc-800 dark:text-zinc-200">{{ $e->sector->comuna->parroquia->municipio->nombre }}</span>
                                    <span class="text-zinc-500 text-xs">{{ $e->sector->nombre }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $e->fase === 'FASE 3' ? 'bg-emerald-100 text-emerald-800' : ($e->fase === 'FASE 2' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800') }}">
                                    {{ $e->fase }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-1">
                                    <flux:button wire:click="show({{ $e->id }})" size="sm" variant="ghost" icon="eye" class="text-zinc-500 hover:text-blue-600" />
                                    <flux:button wire:click="edit({{ $e->id }})" size="sm" variant="ghost" icon="pencil-square" class="text-zinc-500 hover:text-amber-600" />
                                    <flux:button @click="confirmAction($wire, {{ $e->id }}, 'delete', '¿Eliminar registro?', 'Esta acción no se puede deshacer.', 'warning', 'Sí, eliminar')" size="sm" variant="ghost" icon="trash" class="text-zinc-500 hover:text-red-600" />
                                </div>
                            </td>
                        </tr>
                    @empty
